...
//
static lbool_dminisat parse_DIMACS( FILE * in, solver* s ) 
{
...
*/
//=================================================================================================
static unsigned new_count_ones( long long num )
{
	unsigned cnt = 0;
...
static void do_checkpoint( unsigned long long int current_value, 
						   unsigned long long int total_value )
{
	char *ckptfile_name;
	FILE *ckptfile;
	int retval;
	...
	if ( !ckptfile_name ) {
		fprintf(stderr, "APP: Could not resolve the checkpoint filename\n");
		DC_finishClient(1);
	}    
	...
	fprintf( ckptfile, "%d", current_value );
	...
	DC_fractionDone( ( double )( current_value ) / ( double )total_value );
}
...
// Solve SAT-problem with solver dminisat
	...
	for ( i = 0; i < UINT_LEN; i++ )
		range += new_count_ones( full_mask[i + 2] ^ part_mask[i + 2] );
	...
	if ( ( last_iteration_done >= 0 ) && ( last_iteration_done < range_val_count ) )
		start_index = last_iteration_done;
	else
		printf( "last_iteration_done is out of range 0 %d", last_iteration_done );
	
	int max_attempt_tmp = 0;
	for ( lint = start_index; lint < range_val_count; lint++ ) 
	{
		nassigns = 0;
		range_mask = 1;
		range_mask_ind = 1;

		// report to BOINC client not always
		if ( ( lint % FRAC_REPORT_EVERY == 0 ) || ( range_val_count < FRAC_REPORT_EVERY ) )
			do_checkpoint( lint + 1, range_val_count );

		for ( i = 2; i < full_mask[1] + 2; i++ ) 
		{
		...
		*/
		...
		s->verbosity		   = 0; // 0 - no info
		s->corevars_activ_type = 1;
		s->activ_type_attempts = 0;

		st = solver_solve( s, assigns, assigns + nassigns );
		// try othet acriv types
		if ( s->activ_type_attempts == 1 ) 
		{
			printf( "attempt 1 \n");
			s->corevars_activ_type = 0;
			solver_cleardb( s );
			st = solver_solve( s, assigns, assigns + nassigns );
		}
		if ( s->activ_type_attempts == 2 ) 
		{
			printf( "attempt 2 \n");
			s->corevars_activ_type = 10;
			solver_cleardb( s );
			st = solver_solve( s, assigns, assigns + nassigns );
		}
		if ( s->activ_type_attempts == 3 ) 
		{
			printf( "attempt 3 \n");
			s->corevars_activ_type = 100;
			solver_cleardb( s );
			st = solver_solve( s, assigns, assigns + nassigns );
		}
		if ( max_attempt_tmp < s->activ_type_attempts )
		    max_attempt_tmp = s->activ_type_attempts;
		//printf( "\n End of solver_solve \n" );
	...
	printf( st == l_True_dminisat ? "SATISFIABLE\n" : "UNSATISFIABLE\n" );
	
	*max_attempt = max_attempt_tmp;
