...
static boolean clause_is_lit   (clause* c) { return ((unsigned long)c & 1);                              }
...
static inline boolean enqueue(solver* s, lit l, clause* from)
{
    lbool_dminisat* values = s->assigns;
//    vecp* wlists = s->wlists;
    int    v      = lit_var(l);
...
static boolean solver_lit_removable(solver* s, lit l, int minl)
{
...
static lbool_dminisat solver_search( solver* s, int nof_conflicts, int nof_learnts )
{
    ...
    for ( ; ; )
	{
        ...
        if ( confl != 0 )
		{
			...
			if ( conflictC >= MAX_NOCONFL ) // if time limit try another activ type
			{
				s->activ_type_attempts++;
				//printf( "\n s->activ_type_attempts %d \n", s->activ_type_attempts );
				return l_False_dminisat; // fast exit
			}
...
boolean solver_addclause( solver* s, lit* begin, lit* end )
{
    lit *i,*j;
    int maxvar;
    lbool_dminisat* values;
...
boolean   solver_simplify(solver* s)
{
    clause** reasons;
    int type;
...
boolean solver_solve( solver* s, lit* begin, lit* end )
{
	...
	if ( ( !s->IsHardProblem ) && ( s->size > s->core_len ) ) 
	{	
		if ( s->corevars_activ_type == 0 ) // from 1st to last by decreasing 
		{
			for ( i = 0; i < s->core_len ; i++ ) 
			{
				s->activity[i] = s->core_len - i;
				if ( s->orderpos[i] != -1 )
					order_update( s, i );
			}
		}
		else if ( s->corevars_activ_type > 0 ) // equal activ to all vars
		{
			...
			for ( i = 0; i < s->core_len ; i++ ) 
			{
				s->activity[i] = s->corevars_activ_type;
				if ( s->orderpos[i] != -1 )
					order_update( s, i );
			}
		}
...
//        switch (lit_sign(*i) ? -(lbool_dminisat)wlists[lit_var(*i) << 1].dummy : (lbool_dminisat)wlists[lit_var(*i) << 1].dummy){
        switch (lit_sign(*i2) ? -(lbool_dminisat)values[lit_var(*i2) << 1] : (lbool_dminisat)values[lit_var(*i2) << 1])
		{
